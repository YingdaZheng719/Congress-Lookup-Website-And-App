...
class HouseViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchControllerDelegate, UISearchResultsUpdating, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request("http://104.198.0.197:8080/legislators?chamber=house&per_page=all").validate().responseJSON { response in
    ...
    func searchBarCancelButtonClicked(_ searchBar: UISearchBar) {
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .search, target: self, action: #selector(HouseViewController.search_btn(_:)))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "HouseTableCell", for: indexPath) as! HouseTableViewCell
