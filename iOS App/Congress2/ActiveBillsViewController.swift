...
class ActiveBillsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchControllerDelegate, UISearchResultsUpdating, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request("http://104.198.0.197:8080/bills?history.active=true&per_page=50").validate().responseJSON { response in
    ...
    func searchBarCancelButtonClicked(_ searchBar: UISearchBar) {
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .search, target: self, action: #selector(ActiveBillsViewController.search_btn(_:)))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ActiveBillsCell", for: indexPath) as! ActiveBillsTableViewCell
